...
- **Added:** 2026-05-11
- **Started:** 2026-05-14
- **Completed:** 2026-05-14 — branch `feat/ask-agent`. New `app/pages/10_ask.py` plus four new helpers in `app/lib/queries.py`: `get_anthropic_client()` (env-key-driven optional dep, None-or-client), `compile_schema_summary_from_manifest()` (pure dict → text renderer, deterministic ordering for cache stability) + `compile_schema_summary()` wrapper keyed on manifest mtime, `validate_sql()` (three-layer gate: sqlparse statement check + token-walk for DDL/non-SELECT DML + regex-on-FROM/JOIN for `analytics_marts.*`-only), and `execute_safe_sql()` (transaction-scoped `SET LOCAL statement_timeout = '10s'` + LIMIT 10000 injection). Page uses **Claude Sonnet 4.6** (`claude-sonnet-4-6`) with **prompt caching** on the system block (rules + 12.5KB schema + tool defs); subsequent questions in the 5-minute TTL window read cache at ~0.1× cost. Tool-use design: two tools — `run_sql(query, rationale, chart_hint)` for the happy path and `ask_clarification(question)` for genuine ambiguity — typed contract eliminates regex extraction. Schema summary is parsed from `transform/target/manifest.json` so every mart description + column description from `schema.yml` flows through verbatim; a `dbt parse` auto-refreshes via mtime-keyed cache. **5 surprises:** (a) the initial `_add_limit_if_missing` wrapped every SQL in `SELECT * FROM (...) AS _limited LIMIT N`, which Postgres rejects for top-level WITH queries (`SELECT * FROM (WITH cte AS …) AS x` is invalid) — fixed to append LIMIT N to the final SELECT for WITH-prefixed SQL and wrap otherwise; (b) the validator must reject `SELECT 1` (no qualified table ref) even though it's syntactically harmless, because it would otherwise let probe queries through; (c) sqlparse classifies WITH … SELECT as `stmt_type == "SELECT"`, so the single-SELECT check naturally allows CTEs without special-casing; (d) column alias refs like `m.day` look identical to schema-qualified `analytics_marts.x` in a regex over the whole SQL, so the schema gate uses `\b(?:FROM|JOIN)\s+\w+\.\w+` specifically — alias.column never matches; (e) re-ingest required mid-session because `raw.*` was empty before the live walkthrough (likely a stale prior wipe; the BACKLOG-tracked non-destructive fixture is in place but I didn't dig into root cause). **38 new unit tests** in `tests/test_ask.py` (validate_sql: 5 happy + 13 reject incl. parametrized DDL/DML; compile_schema_summary: 7 incl. sort-stability for cache invariance; get_anthropic_client: 3; _add_limit_if_missing: 7 incl. the WITH-CTE branch). End-to-end simulation against real local data (post-reingest: 288K HR samples, 78 workouts, 29 sleep nights) verified all 6 example questions return non-empty plausible results (HRV by week: 6 rows, recovery distribution: 4 rows incl. 12 well_recovered / 11 strained, top Zone 2 workouts: 10 rows topping at 78.85min Cycling on Mar 31, sleep on workout vs rest days: 6.52h vs N/A, below-normal HRV days: 1 row at z=-1.41 on Apr 12, training load: 30 rows). Local: ruff/format/mypy clean, pytest 181/181 (143 prior + 38 new), dbt build PASS=159 TOTAL=160 (1 NO-OP exposure), page-import smoke auto-covered via glob (13 pages, +1 from 10 prior).
...
- **Added:** 2026-05-12
- **Started:** 2026-05-13
- **Completed:** 2026-05-13 — branch `refactor/sleep-split-main-from-naps`. New intermediate `int_sleep_periods` gap-splits each night's segments into contiguous sleep periods using a 120-minute threshold computed on the *asleep-only* segment chain (first attempt computed gaps on all segments, which missed the 5.4-hour single `awake` stage on Apr 15→16 — a long awake segment is itself a non-gap by chronological adjacency, so the asleep-only filter was the correct fix; the longest period per night is `is_main_period` with tiebreaker earliest-start; periods under 5 minutes are filtered as noise). `mart_sleep_nights` now consumes `int_sleep_periods` filtered to `is_main_period=true`; non-main periods with any actual sleep flow into the new `mart_sleep_naps` (grain: one row per nap, with `nap_date` = the calendar date the nap started, distinct from `night_date` which preserves upstream noon-to-noon attribution for joins back). Streamlit `12_sleep.py` gains a "Naps in the trend window" section + caption tweak; `app/lib/queries.py` adds `sleep_naps()`. Real-data verification on 29 nights: Apr 16 drops 12.8h → 11.0h asleep, 13 → 10 awakenings, score 57.8 → 71.0, efficiency 87.4% (was depressed by the nap collision); Apr 14 drops 14.1h → 12.0h asleep, 15 → 13 awakenings, score 40.9 → 54.2 — partial fix because the 28-minute wake between the main sleep ending 07:41 and a 2h morning extension at 08:09 is below any reasonable gap threshold, so Apr 14 is one genuine 12h sleep stretch in the data (sick day / lazy weekend; data is honest). Four naps surfaced in the new mart: Mar 23 (29min noon nap), Mar 25 (37min noon nap), Apr 13 (126min afternoon nap that previously contaminated Apr 14's rollup), Apr 15 (110min afternoon nap separated from Apr 16's main sleep by the 5.4h awake gap). `mart_recovery_state` row count + recovery_signal distribution verified unchanged (32 rows; 2 insufficient_data / 7 neutral / 11 strained / 12 well_recovered) — the public-API mart doesn't depend on sleep marts so the contract is structurally untouchable from this refactor. dbt build green: PASS=159 TOTAL=160 (1 NO-OP exposure). pytest 140/140 (one pre-existing weather-loader failure on main excluded — unrelated, will file separately). ruff/format/mypy all clean.
...
- **Added:** 2026-05-11
- **Started:** 2026-05-13
- **Completed:** 2026-05-13 — branch `claude/anomaly-notification-pipeline`. New `ingest/notifications/` package: `rules.py` (pure YAML-driven evaluator with two kinds — `transition_to` and `consecutive(N)` — both designed to fire ONCE per state-change rather than every day during a streak), `transport.py` (stdout + Pushover via stdlib urllib, no new top-level deps beyond `pyyaml` which was already transitive via dbt-core), `notify.py` (orchestrator: pulls trailing window from `analytics_marts.mart_recovery_state`, claims a row in `raw.notification_log` via `INSERT … ON CONFLICT DO NOTHING`, and only fires transports when the claim succeeds — so re-running `weekly_load` on the same day is a structural no-op, not a soft check). Compared the latest mart state to *yesterday's row in the mart* instead of a separate "prior-run state" file: the mart is keyed on `day`, not `run_id`, so two runs the same day produce identical results, and dedup belongs at the log table. New DDL `raw.notification_log (rule_name, day) PK` in `scripts/init_raw_schema.sql`. Default rules at `config/notification_rules.yaml` ship one of each kind (red transition into strained + 3-consecutive-strained-day streak). Wired into `weekly_load` as a non-fatal `@task` after `run_dbt_build`, following the existing weather/calendar try/except shape — a Pushover outage or empty mart never blocks dbt. Pushover chosen over Slack/iMessage because the BACKLOG framing is *interruption* and only phones interrupt; iMessage MCP isn't available from a Prefect runtime; Pushover is one HTTPS POST with no OAuth. `NOTIFY_DRY_RUN=true` forces stdout-only and tags the log row with `transport='dry_run'` so post-mortems separate rehearsals from real fires. 38 new tests: 27 pure-rule unit tests (full coverage of `load_rules` validation, both rule kinds' edge cases including the suppress-on-streak-extension invariant, and `format_message` template fallback), 6 transport unit tests (monkeypatched urlopen, no real HTTP), 5 integration tests via `raw_test_engine` proving the BACKLOG's "one and only one notification" acceptance criterion — second invocation on the same day fires zero. Local verification: ruff clean, mypy clean, pytest 141/141 (103 prior + 38 new), dbt build 139/140 PASS (1 NO-OP exposure).
